...
Let $A$ be any $n \times k$ function of $X$ of such that $A'X = I_k$. The estimator $\tilde{\beta} = A'Y$ is unbiased: $E[\tilde{\beta}|X] = E[A'Y|X] = E[A'(X\beta + e)|X]= E[A'X\beta|X] + E[A'e|X] = \beta$. The variance of $\tilde{\beta}$ is $\var[\tilde{\beta}|X] = A' \Omega A$