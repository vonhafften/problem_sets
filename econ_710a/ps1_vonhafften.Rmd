...
We cannot use the Lindeberg-Levy CLT or CMT to find the asymptotic distribution because they require that the statistic is shifted by its population expection not its sample average.

(iii) Show that $\max_{1 \le i \le n} X_i \to_p 1$ if $X \sim uniform(0, 1)$.

...

(iv) Show that $\Pr(\max_{1 \le i \le n} X_i > M) \to_p 1$ for any $M \ge 0$ if $X \sim exponential(1)$.
...
No. Conditional on $W=1$, $X_i$ and $Y_i$ are perfectly coorelated, so $Cov(X_i, Y_i) = 1$.  Conditional on $W=-1$, $X_i$ and $Y_i$ are perfectly inversely coorelated, so $Cov(X_i, Y_i) = -1$.  Since $\Pr(W=1)=\Pr(W=-1)=1/2$, the unconditional covariance is zero, but $V$ cannot converge to a joint normal distribution with zero covariance because the covariance is either always going to one or minus one depending on the value of $W$.
