...
Assuming that $\mu = 1$, $X \sim N(1, 1) \implies \sqrt{n}(\bar{X}_n-1) \sim N(0, 1)$ by the CLT. Furthermore, $|\sqrt{n}(\bar{X}_n-1)| \sim |N(0,1)|$.  In addition,

$$
\sqrt{n}(\bar{X}_n-1) \sim N(0, 1) \implies \sqrt{n}\bar{X}_n \sim N(\sqrt{n}, 1) \implies |\sqrt{n}\bar{X}_n| \sim |N(\sqrt{n}, 1)| \implies |\sqrt{n}\bar{X}_n| \sim |N(0, 1)+\sqrt{n}|
$$

Thus, $\min\{ |\sqrt{n}\bar{X}_n|,|\sqrt{n}(\bar{X}_n-1)|\} \sim \min\{|Z|, |Z + \sqrt{n}|\}$ where $Z \sim N(0, 1)$.  Since $Z$ is symmetic ($Z$ and $-Z$ have the same distribution), $|Z + \sqrt{n}|$ and $|-Z+\sqrt{n}| =|(-1)*(Z-\sqrt{n})| = |-1||Z-\sqrt{n}| = |Z-\sqrt{n}|$ have the same distribution.  So $T =\min\{ |N(\sqrt{n}, 1)|, |\sqrt{n}(\bar{X}_n-1)|\} \sim \min\{|Z|, |Z - \sqrt{n}|\}$.  Therefore, $\Pr(T > c | \mu = 1) = F_{T|\mu=1}(c)= \alpha$ by definition of $c$.  Thus, the size of the test $\phi_n = 1(T > c)$ is $\alpha$. $\square$
