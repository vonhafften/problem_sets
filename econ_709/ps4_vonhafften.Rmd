...
Most of the problems assume a random sample $\{X_1, ..., X_n\}$ from a common distribution $F$ with density $f$ such that $E(X) = \mu$ and $Var(X) = \sigma^2$ for generic random variable $X \sim F$. The sample mean and variances are denoted $\bar{X}_n$ and $\sigma^2 = n^{-1} \sum_{i=1}^n (X_i - \bar{X}_n)^2$, with the bias corrected variance $s^2 = (n-1)^{-1} \sum_{i=1}^n (X_i - \bar{X}_n)^2$.
...
Consider $\hat{\mu}_k = \frac{1}{n} \sum_{i=1}^n X_i^k$, which is unbiased:
...
Consider $\hat{m}_k = \frac{1}{n} \sum_{i=1}^n (X_i-\bar{X}_n)^k$. If $g(x) = x^k$ is convex:

$$
E(\hat{m}_k) = E \Bigg( \frac{1}{n} \sum_{i=1}^n (X_i-\bar{X}_n)^k \Bigg)=  \frac{1}{n} \sum_{i=1}^n E[(X_i-\bar{X}_n)^k] \ge  \frac{1}{n} \sum_{i=1}^n E(X_i-\bar{X}_n)^k=\frac{1}{n} \sum_{i=1}^n (E(X_i)-E(\bar{X}_n))^k=\frac{1}{n} \sum_{i=1}^n (\mu-\mu)^k = 0
$$
