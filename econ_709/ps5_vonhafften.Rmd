...
Notice that the $E(X_i^k) = \mu_k$ and $Var(X_i^k) = E(X_i^{2k})-(\mu_k)^2 = \mu_{2k}-\mu_k^2$.  Thus, by the central limit theorem, $\sqrt{n}(\hat{\mu}_k-\mu_k)\to_d N(0, \mu_{2k}-\mu_k)$.
...
From 6(b), we know that $\sqrt{n}(\hat{\mu}_k-\mu_k)\to_d N(0, \mu_{2k}-\mu_k)$. Define $g(x)=x^{1/k}$ for some $k \in \N$.  Notice that $g$ is continuous for all values of $k$.  Furthermore, $g(\hat{\mu}_k) = \hat{m}_k$, $g(\mu_k) = m_k$, and $g'(\mu_k) = (1/k)\mu_k^{(1-k)/k} = (1/k)m_k^{1-k}$. Thus, by the Delta Method,  

\begin{align*}
\sqrt{n}(g(\hat{\mu}_k)-g(\mu_k)) &\to_d N(0, (g'(\mu_k))^2(\mu_{2k}-\mu_k) \\
\implies \sqrt{n}(\hat{m}_k-m_k) &\to_d N(0, ((1/k)m_k^{1-k})^2(m_{2k}^k-m_k^k)) \\
\implies \sqrt{n}(\hat{m}_k-m_k) &\to_d N\Bigg(0, \frac{m_k^{2}(m_{2k}^k-m_k^k)}{k^2m_k^{2k}}\Bigg)
\end{align*}
...
If $\mu = 0$, then $\beta=(0)^2=0$ and the variance of the asymptotic distribution is zero, so it become a degenerate distribution at its mean:
...
(c) Improve on the previous answer. Under the assumption $\mu = 0$, find the asymptotic distribution of $\sqrt{n}\hat{\beta}$.

Define $g(x) = x^2$.  Notice that $g$ is continuous, $\beta = g(\mu) = 0^2=0$, and $\hat{\beta} = g(\hat{\mu})$. Furthermore $g'(0) = 2(0) = 0$. By the Delta Method,

$$
\sqrt{n}\hat{\beta} \to_d N(0, (0)^2v^2)
$$

Which implies that $\sqrt{n}\hat{\beta}$ is a degenerate probability distribution at zero, so $\hat{\beta} = 0$.
