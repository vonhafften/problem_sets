...
Notice that $u$ is differentiable. Define $f(x) = \sum_{i=1}^k a_i^{\frac{1}{s}} x_i^\frac{s-1}{s}$. Note that maximizing $u(x)=(f(x))^\frac{s}{s-1}$ is the same as maximizing $f(x)$ when $s > 1$, and the same as minimizing $f(x)$ when $s < 1$.  Notice that $f$ is concave: $\frac{\partial^2 f}{\partial^2 x_i} = -a_i^{\frac{1}{s}}  \frac{s-1}{s^2}x_i^{\frac{-1-s}{s}} < 0$.  So the Kuhn-Tucker Conditions solve the consumer problem. Define $\Lf = \sum_{i=1}^k a_i^{\frac{1}{s}} x_i^\frac{s-1}{s} - \lambda(w-p\cdot x) - \mu \cdot x$

\begin{align*}
\frac{\partial \Lf}{\partial x_i} &= 0 \\
\implies \frac{s-1}{s}a_i^{\frac{1}{s}} x_i^{\frac{s-1}{s}-1} - \lambda p_i -\mu_i &= 0 \\
\implies  x^*_i &= \Bigg(\frac{s}{s-1} \frac{\lambda p_i + \mu_i}{a_i^{1/s}} \Bigg)^{-s} \\
\text{and } \lambda &= \frac{s-1}{p_is}a_i^{1/s} x_i^{\frac{s-1}{s}-1} -\frac{\mu_i}{p_i}
\end{align*}

Thus $x^*_{i}$ is positive $\forall i \implies \mu_{i}=0$ $\forall i$ by complementary slackness.  Therefore, we know that $\lambda > 0 \implies w - p \cdot x = 0$, so Walras' Law holds:

$$
\sum_{i=1}^np_i\Bigg(\frac{s}{s-1} \frac{\lambda p_i}{a_i^{1/s}} \Bigg)^{-s} =w
$$
