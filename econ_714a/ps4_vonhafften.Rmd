...
The total cost for firm $i$ in sector $k$ is:

$$
TC_{ik} = WL_{ik} = \frac{WC_{ik}}{A_{ik}}
$$
...
Firm $i$'s mark-up is the difference between their price and marginal cost:
...
- \frac{W}{A_{ik}}\\
&= \frac{W}{A_{ik}}\Bigg( \frac{(\theta - \rho)s_{ik}-\theta }
{(\theta - \rho)s_{ik} +1 - \theta} -1\Bigg)\\
&= \frac{W}{A_{ik}}\Bigg( \frac{ -1 }
{(\theta - \rho)s_{ik} +1 - \theta} \Bigg)\\
\end{align*}

Holding $s_{ik}$ constant, more productive firms charge higher mark-ups:

$$
\frac{\partial MU_{ik}}{\partial A_{ik}}
= \frac{W}{A_{ik}^2}\Bigg( \frac{ -1 }
{(\theta - \rho)s_{ik} +1 - \theta} \Bigg) (-1)\\
= \frac{W}{A_{ik}^2}\Bigg( \frac{ 1 }
{(\theta - \rho)s_{ik} +1 - \theta} \Bigg) > 0
$$
...
n_k <- 20
k <- 100000
a <- as.matrix(exp(rnorm(n = k * n_k)), ncol = n_k, nrow = k)

# real wage
w0 <- mean(a)
w1 <- 10

p0 <- theta / (theta - 1) * (w0 / a)
p1 <- p0 + 100

# Solves general equilibrium fixed point problem for wage
while (TRUE) {
  
  # Solves sectoral equilibrium fixed problem for prices
  while (TRUE) {
