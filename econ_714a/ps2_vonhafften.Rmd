...
where $\phi \equiv A\bar{K}^{\alpha-1}$.  From the capital steady state, we know that $\delta  \alpha \phi + 1 - \delta = \frac{1}{\beta}$.  
...
&= \frac{1}{\beta}k_t - \delta \frac{\bar{C}}{\bar{K}}c_t
\end{align*}

Turning to the Euler equation, define $X_t := (1-\delta) \frac{I_{t+1}}{I_{t}} \implies \bar{X} = 1 -\delta$ and:

\begin{align*}
X_t &= (1-\delta) \frac{I_{t+1}}{I_{t}}\\
\implies
x_t &= i_{t+1} - i_t \\
&= (\phi \alpha k_{t+1} - \frac{\bar{C}}{\bar{K}}c_{t+1}) - (\phi \alpha k_t - \frac{\bar{C}}{\bar{K}}c_t) \\
&= \phi \alpha (\frac{1}{\beta}k_t - \delta \frac{\bar{C}}{\bar{K}}c_t) - \frac{\bar{C}}{\bar{K}}c_{t+1} - \phi \alpha k_t + \frac{\bar{C}}{\bar{K}}c_t \\
&= \phi \alpha(\frac{ 1 }{\beta}-1)k_t + (1 - \phi \alpha \delta) \frac{\bar{C}}{\bar{K}}c_t - \frac{\bar{C}}{\bar{K}}c_{t+1} \\
\end{align*}

Define $Z_t := \delta \frac{\alpha Y_{t+1}}{I_t} \implies \bar{Z} = \delta \alpha \phi$ and:

\begin{align*}
Z_t &= \delta \frac{\alpha Y_{t+1}}{I_t} \\
\implies z_t &= y_{t+1} - i_t \\
&= \alpha k_{t+1} - (\phi \alpha k_t - \frac{\bar{C}}{\bar{K}}c_t) \\
&= \alpha (\frac{1}{\beta}k_t - \delta \frac{\bar{C}}{\bar{K}}c_t) - \phi \alpha k_t + \frac{\bar{C}}{\bar{K}}c_t \\
&= \alpha (\frac{1}{\beta} - \phi) k_t + (1 - \delta) \frac{\bar{C}}{\bar{K}}c_t \\
\end{align*}

Thus, the Euler equation can be approximated by:

$$
C_t^{-1}  = C_{t+1}^{-1}  \beta \Bigg[(1-\delta) \frac{I_{t+1}}{I_{t}} + \delta \frac{\alpha Y_{t+1}}{I_t}\Bigg] \\
C_{t+1}  = C_t  \beta [X_t + Z_t] \\
\bar{C}(1+c_{t+1})  = \bar{C}(1+c_t)  \beta [\bar{X}(1+x_t) + \bar{Z}(1+z_t)] \\
(1+c_{t+1})  = (1+c_t)  \beta [(1-\delta)(1+i_{t+1} - i_t) + \delta \alpha\phi(1+y_{t+1} - i_{t})] \\
(1+c_{t+1})  = (1+c_t)  \beta [(1 - \delta +\delta \alpha \phi)(1-i_t) +(1-\delta)(i_{t+1}) + \delta \alpha\phi(y_{t+1} )] \\
(1+c_{t+1})  = (1+c_t)  \beta [(1/\beta)(1-i_t) +(1-\delta)(i_{t+1}) + \delta \alpha\phi(\alpha k_{t+1} )]
$$
...
C_t^{-1}  = C_{t+1}^{-1}  \beta \Bigg[(1-\delta) \frac{I_{t+1}}{I_{t}} + \delta \frac{\alpha Y_{t+1}}{I_t}\Bigg] \\
\implies
\bar{C}(1-c_{t+1})  = \bar{C}(1-c_{t})  \beta \Bigg[(1-\delta) \frac{\bar{I}(1-i_{t+1})}{\bar{I}(1-i_{t})} + \delta \frac{\alpha \bar{Y}(1-y_{t+1})}{\bar{I}(1-i_{t})}\Bigg] \\
\implies
(1-c_{t+1})  = (1-c_{t})  \beta \Bigg[(1-\delta) \frac{(1-i_{t+1})}{(1-i_{t})} + \delta \frac{\alpha A\bar{K}^\alpha(1-y_{t+1})}{\bar{K}(1-i_{t})}\Bigg] \\
\implies
(1-c_{t+1})  = (1-c_{t})  \beta \Bigg[(1-\delta) \frac{(1-i_{t+1})}{(1-i_{t})} + \delta \frac{\alpha \phi(1-y_{t+1})}{(1-i_{t})}\Bigg] \\
\implies
$$
