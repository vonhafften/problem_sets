...
To argue that the EE represents the optimal path, assume we're on the equilibrium path for capital $\{K_t\}_{t=0}^\infty$ and a deviation at some $t$ such that $\tilde{K}_{t+1} < K_{t+1}$. To achieve such a deviation, $\tilde{C}_t = C_t + \Delta$ for some $\Delta > 0$. The feasibility constraint implies that $\tilde{I}_t = I_t - \Delta$.  Thus, capital in period $t+1$ can be approximated as:
...
&= K_{t+1} -  K_{t+1}\frac{\delta \Delta}{I_t}
\end{align*}

With lower capital, production in $t+1$ is less:
...
&= Y_{t+1} - Y_{t+1}\frac{ \alpha \delta \Delta}{I_t}
\end{align*}
