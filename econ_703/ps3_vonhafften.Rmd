...
A basis of $F_X$ is $\mathcal{B}_1 =\{f_1, f_2, f_3\}$ where

\begin{align*}
f_1 &= \begin{cases} 1, x=1\\ 0, x=0\\ 0, x=-1 \end{cases}\\
f_2 &= \begin{cases} 0, x=1\\ 1, x=0\\ 0, x=-1 \end{cases}\\
f_3 &= \begin{cases} 0, x=1\\ 0, x=0\\ 1, x=-1 \end{cases}
\end{align*}
...
A basis of Im $T$ is $\mathcal{B}_2 =\{f_4, f_5\}$ where

\begin{align*}
f_4 &= \begin{cases} 1, x=\{-1, 1 \} \\ 0, x=0 \end{cases}\\
f_5 &= \begin{cases} 0, x=\{-1, 1 \} \\ 1, x=0 \end{cases}\\
\end{align*}

Thus, the rank of $T$ is the dimension of the image of $T$:

rank $T=$ dim $($Im $T)=$ dim $(\mathcal{B}_2) = 2$.
...
In (a), we found that $\{\vec{u}, \vec{v}\} = \{(3, 5, -4, 0)^T, (3, 0, -4, 5)^T\}$ spans vector space $X$, so any $x \in X$ $\exists a, b \in \mathbb{R}$ such that $x = a\vec{u} + b \vec{v}$.
...
Thus, $\vec{u}$ and $\vec{v}$ are linearly independent.  Thus, $\{\vec{u}, \vec{v}\}$ forms a basis for $X$. Since the dimension of $X$ is the cardinality of a basis for $X$, dim $X = 2$.
