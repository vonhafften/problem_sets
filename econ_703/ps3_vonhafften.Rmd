...
Since an arbitrary $f \in$ Im $T$ can be written as $f(x) = ag_1(x)+bg_2(x)$, $\mathcal{B}$ spans Im $T$. $\mathcal{B}$ is also linearly independent because, if $c, d \in \mathbb{R}$, $cg_1(x)+dg_2(x)=0 \iff c=d=0$.  Thus, $\mathcal{B}$ is a basis for Im $T$. The rank of $T$ is the dimension of the image of $T$:
