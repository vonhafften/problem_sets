...
where $A \in M_{n \times n}$ and $y(0) \in \R^n$ are given. When $n=1$ we know that solution to Eq. (1) is $y(t)=e^{At}y(0)$. Turns out, it remains the same when $n > 1$, thus, it involves exponent of a matrix, which we have not defined before. To properly define $e^{At}, A \in M_{n \times n}$ we use Taylor expansion and say that

$$
e^{At} = I+A+\frac{1}{2}A^2t^2+\frac{1}{6}A^3t^3+...=\sum_{k=0}^{\infty} \frac{1}{k!}A^kt^k.
$$

To calculate $e^{At}$ we will use diagonalization. Suppose that $A=Pdiag\{\lambda_1, ..., \lambda_n\}P^{_1}$, so that $A^k=Pdiag\{\lambda^k_1, ..., \lambda^k_n\}P^{_1}$ and

\begin{align}
e^{At} = 
P \Big ( \sum_{k=0}^{\infty} \frac{1}{k!}diag\{t^k\lambda^k_1, ..., t^k\lambda^k_n\} \Big ) P^{-1} = 
P \Big ( diag \Big \{\sum_{k=0}^{\infty} \frac{1}{k!}t^k\lambda^k_1, ..., \sum_{k=0}^{\infty} \frac{1}{k!}t^k\lambda^k_n \Big \} \Big ) P^{-1} = Pdiag\{e^{t\lambda_1}, ..., e^{t\lambda_n}\}P{-1}
\end{align}
