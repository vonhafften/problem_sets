...
Proof: Observe that, if $f'(x)<0$ for any $x \in B_\varepsilon(x_0) \setminus \{ x_0 \}$, then $f'(x_0)=0$ or $f'(x_0)<0$.

Assume $f'(x_0)<0$. Assume for the sake of a contradiction that $x_0$ is a local maximum or minimum. Then $f'(x_0)=0$. $\Rightarrow \Leftarrow$. $x_0$ cannot be a local maximum or minimum.
...
Proof: Let $f : X \to \R^n$ be a continuously differentiable function on the open set $X \subset \R^n$. Choose $x \in X$ and $\varepsilon > 0$. Define $B_\varepsilon(x)$ as the open ball around $x$ using Euclidean distance. Since $f$ is continuously differentiable on $X$, it is continuously differentiable on $B_\varepsilon(x) \cap X$. Choose $w, y \in B_\varepsilon(x) \cap X$. Since $f$ is differential, there exists $z_i \in \R^n$ such that $f^i(w)-f^i(y) = Df^{i}(z_i)(w^i - y^i)$ where $z_i \in \{\alpha w + (1 - \alpha)y | \alpha \in [0, 1]\}$ for all $i=1,...,n$. Thus, $\sum_{i=1}^{n}f^i(w)-f^i(y) = \sum_{i=1}^{n}Df^{i}(z_i)(w^i - y^i)$.
...
The characteristic polynormal is $(3/2-\lambda)(2-\lambda)-1 = 0 \implies \lambda^2 -7/2\lambda+2 \implies \lambda_1 = 7/4 + \sqrt{17}/4, \lambda_2 = 7/4 - \sqrt{17}/4$. Since both eigenvalues are positive, $f$ has local minima at $(1/4, 1/8)$ and $(-1/4, -1/8)$.  Since $f(1/4, 1/8)=f(-1/4, -1/8)$, $(1/4, 1/8)$ and $(-1/4, -1/8)$ are also global minima.  Since $f$ has no local maxima, it has no global maxima.
