...
For any $\delta > 0$, consider all $k > K$ where $K = \frac{(\pi/\delta)^2}{2\pi}$...............

Thus, $k \to \infty$, $|x-y| \to 0 \implies |x-y|<\delta$ .  Thus, $f(x) = \cos x^2$ is not uniformly continuous on $\mathbb{R}$. $\square$
...
ADD WRAP UP OF INDUCTION

For $\varepsilon > 0$, choose $N=\log_2{\frac{b-a}{\varepsilon}}+1$, $|\frac{b - a}{2^{n-1}} - 0| < \varepsilon$ for $n \ge N$.  $\implies \{u_n - l_n\} \to 0$. Since from (a), we know $u_n \to u$ and $l_n \to l$, this result implies that $u=l$. SQUEEEEEEEEZE $\square$
