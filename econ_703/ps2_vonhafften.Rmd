...
Proof: Notice that, for $f(m_n) > 0$, $u_{n+1} - l_{n+1} = m_n - l_n = (l_n+u_n)/2 - l_n = (u_n - l_n)/2$.  Similarly, for $f(m_n) < 0$, notice $u_{n+1} - l_{n+1} = u_n - m_n = u_n - (l_n+u_n)/2 = (u_n - l_n)/2$. So, for $n < N$ such that $f(m_N) = 0$, $u_{n+1} - l_{n+1} = (u_n - l_n)/2$.
...
For $\varepsilon > 0$, choose $N$ such that $\forall n > N$, $|\frac{b - a}{2^n} - 0| < \varepsilon \implies \{u_n - l_n\} \to 0$. Since from (a), we know $u_n \to u$ and $l_n \to l$, this result implies that $u=l$. $\square$
...
Now, I will show that $f(l_n) < 0 < f(u_n)$ for all $n \in \mathbb{N}$ using induction. For the base step, $f(l_1) =f(a) < 0 < f(b) = f(u_1)$. For the induction step, assume that $f(l_n) < 0 < f(u_n)$.  If $f(m_n) > 0$, then $f(l_n) = f(l_{n+1}) < 0 < f(m_n) = f(u_{n+1})$.  If $f(m_n) < 0$, then $f(l_{n+1}) = f(m_n) < 0 < f(u_n) = f(u_{n+1})$. Thus, $f(l_n) < 0 < f(u_n)$.  Since taking limits preserve weak inequalities: $f(l) \le 0 \le f(u) \implies f(c) \le 0 \le f(c) \implies f(c) = 0$. $\square$
