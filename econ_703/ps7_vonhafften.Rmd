...
Proof: Assume a function $f: X \to \R$ is concave where $X$ is a convex set in $\R^n$.  To show that its hypograph is a convex set, we need to show that, for any $\lambda \in [0, 1]$ and $(y', x'), (y'', x'') \in \hyp f$, $\lambda (y', x') + (1-\lambda) (y'', x'') = (\lambda y' + (1-\lambda)y'', \lambda x' + (1-\lambda)x'')  \in \hyp f$.  First, notice that since $X$ is convex, $\lambda x' + (1-\lambda) x'' \in X$. Since $f$ is concave, $f(\lambda x' + (1-\lambda)x'') \le (1-\lambda)f(x')+\lambda f(x'') \le (1-\lambda)y'+\lambda y''$. Thus, $\lambda (y', x') + (1-\lambda) (y'', x'') \in \hyp f$.
...
Proof: Let $X$ and $Y$ be disjoint, closed, and convex sets in $\R^n$ and $Y$ be compact. Let $Z := X-Y = \{z \in \R^n | z=x-y$ for some $x \in X, y \in Y\}$.  The set $Z$ is convex and $\bar{0} \notin Z$ because $X \cap Y = \emptyset$.  By the theorem on slide x of the lecture notes, there exists a hyperplane $H(p, \beta)$ that strictly separates $Z$ and $\{\bar{0}\}$.  Thus, for all $z \in Z, x \in X, y \in Y$,
...
Define $f:Y \to \R$ as $f(y) = p \cdot y$.  Since $f$ is continuous and Y is compact, $f$ attains its maximum on $Y$ by the extreme value theorem.  Set $\beta^* = \max(f(y))$ for all $y \in Y$.  Thus, $\beta^* = \max\{p \cdot y | y \in Y\} < p \cdot x$ for all $x \in X$.  
