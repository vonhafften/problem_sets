...
function free_entry(j::Float64, κ::Float64, ζ::Float64)
    ...
    if κ > j
        return 0.0
    else # binding free entry condition
...
function Solve_nonterminal_periods!(R::Results; progress::Bool = false)
    ...
    for t = (P.T-1):-1:1
        ...
        # compute firm value
        # cycle over levels of human capital and piece rates
        for (i_h, h) = enumerate(G.grid_h), (i_ω, ω) = enumerate(G.grid_ω)
        ...
        # cycle over levels of human capital, cycle over piece rates
        for (i_h, h) = enumerate(G.grid_h), (i_ω, ω) = enumerate(G.grid_ω), (i_b, b) = enumerate(G.grid_b)
            ...
            for (i_b_p, b_p) = enumerate(G.grid_b)
                ...
                # continuation value if becomes unemployed and human capital increase
                value += P.β * P.δ * P.p_H * R.u_vf[t+1, i_b_p, i_h_up]

                # save if better than candidate
                if value > candidate_max_w
                    R.w_vf[t, i_ω, i_b, i_h] = copy(value)
                    R.w_b_pf[t, i_ω, i_b, i_h] = copy(b_p)
                    candidate_max_w = copy(value)
                end
            end
        end
        
        # compute unemployed worker value
        ...
        for (i_b, b) = enumerate(G.grid_b), (i_h, h) = enumerate(G.grid_h)
            ...
            candidate_max_u = -1/eps()

            # cycle over assets tomorrow
            # cycle over piece rates tomorrow
            for (i_b_p, b_p) = enumerate(G.grid_b), (i_ω_p, ω_p) = enumerate(G.grid_ω)
                ...
                # save if better than candidate
                if value > candidate_max_u
                    R.u_vf[t, i_b, i_h]   = copy(value)
                    R.u_b_pf[t, i_b, i_h] = copy(b_p)
                    R.u_ω_pf[t, i_b, i_h] = copy(ω_p)
                    candidate_max_u = copy(value)
                end
            end
        end
        if progress
