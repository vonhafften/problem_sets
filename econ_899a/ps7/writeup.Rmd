...
- Table 1 shows the first stage estimates, second stage estimates, and standard errors for $\rho$ and $\sigma$.
...
kable(cbind(moments, select(summary, starts_with("rho"), starts_with("sigma"))), digits = 3, caption = "Estimates and Standard Errors")
```

- To obtain the standard errors, I computed the following Jacobians:

```{r echo= FALSE}
jacobian_4 <- read_csv("tables/jacobian_4.csv", col_types = cols())
jacobian_5 <- read_csv("tables/jacobian_5.csv", col_types = cols())
jacobian_6 <- read_csv("tables/jacobian_6.csv", col_types = cols())

kable(jacobian_4, col.names = c("d/(d rho)", "d/(d sigma)"), caption = "Jacobian (Mean and Variance)")
kable(jacobian_5, col.names = c("d/(d rho)", "d/(d sigma)"), caption = "Jacobian (Variance and Covariance)")
kable(jacobian_6, col.names = c("d/(d rho)", "d/(d sigma)"), caption = "Jacobian (Mean, Variance, and Covariance)")
```

- I found very low j-test statistics and p-values. 

```{r echo= FALSE}
moments = c("Mean and Variance", "Variance and Covariance", "Mean, Variance and Covariance")

kable(cbind(moments, select(summary, j_test_stat, j_test_p_value)), digits = 5, caption = "J-Test Statistics and P-Values")
```
