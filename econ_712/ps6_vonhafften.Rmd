...
Black lines are the solution to social planner's problem, blue lines are the Ramsey equilibrium, and red lines are the Nash equilibrium.
...
lines(x=alpha, y= log(l_sp) + log(alpha + c_sp) + log(alpha + g_sp), col = "black")
lines(x=alpha, y= log(l_ne) + log(alpha + c_ne) + log(alpha + g_ne), col = "red")
lines(x=alpha, y= log(l_re) + log(alpha + c_re) + log(alpha + g_re), col = "blue")
```
