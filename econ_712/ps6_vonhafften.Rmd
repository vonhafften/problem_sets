...
\begin{align*}
\max_{l, c, g \in \R_+^3} & \ln l + \ln(\alpha + c) + \ln(\alpha + g) \\
\text{s.t. } & l + g + c = 1, g = \tau(1 - l) \text{ and } \tau \in [0, 1]
\end{align*}

Subsistuting in $l = 1 - g -c$, we get

\begin{align*}
\max_{c, g \in \R_+^2} & \ln (1 - g - c) + \ln(\alpha + c) + \ln(\alpha + g) \\
...
4c = 2 - (1 - c -\alpha) -2\alpha \implies
c = \frac{1 -\alpha}{3} > 0
$$
...
$$
\tau = \frac{g}{1 - l} = \frac{\frac{1 -\alpha}{3}}{1- \frac{1 + 2\alpha}{3}}=\frac{1 -\alpha}{2 - 2\alpha}=\frac{1}{2} \in [0,1]
$$
...
In the Ramsey outcome, the government moves first and the household moves second.  Specifically the government chooses $\tau$ and since the individual household cannot change the aggregate $g = (1-\tau)\bar{l}$ is also fixed.  Thus, we first solve the individual household problem conditional on $\tau$ and $g$. The individual household's income is $(1-l)$ and they pay $(1-l)\tau$ in taxes, so their discretionary income is $(1-l)(1-\tau)$.  Thus the household problem is:

\begin{align*}
\max_{c, l \in \R^2_+} & \ln l + \ln(\alpha + c) + \ln(\alpha + g) \\
\text{s.t. } & c \le (1-l)(1-\tau)
\end{align*}

Since the household's income is increasing in $c$, they will consume their entire discretionary income. We can subsitute in $c = (1-l)(1-\tau)$:

$$
\max_{l \in \R^2_+} \ln l + \ln(\alpha + (1-l)(1-\tau)) + \ln(\alpha + g) \\
$$

Thus, the FOC implies
...
$$
\max_{\tau \in [0,1]} \ln \Bigg( \frac{\alpha + (1-\tau)}{2(1-\tau)} \Bigg) + \ln\Bigg(\alpha + \frac{(1-\tau) - \alpha}{2}\Bigg) + \ln\Bigg(\alpha + \frac{\tau(1-\tau) - \tau\alpha}{2(1-\tau)}\Bigg)  \\
=\max_{\tau \in [0,1]} \ln \Bigg( \frac{\alpha - \tau + 1}{2 - 2\tau} \Bigg) + \ln\Bigg(\frac{1-\tau + \alpha}{2}\Bigg) + \ln\Bigg(\frac{2\alpha +(1- 3\alpha) \tau - \tau^2}{2-2\tau}\Bigg)
$$

The FOC implies

$$
\Bigg( \frac{2(1-\tau)}{\alpha + (1-\tau)} \Bigg) + \Bigg(\frac{2}{(1-\tau) + \alpha}\Bigg) 
$$
