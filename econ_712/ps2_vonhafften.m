...
c_bar2 = z * k_bar^alpha - delta * k_bar;

% Jacobian matrix at new steady state
J2_11 = z * alpha * k_bar^(alpha-1) + 1 - delta;
J2_12 = -1;
J2_21 = z * alpha * beta * c_bar * (alpha - 1)*(z * k_bar^alpha+(1-delta)*k_bar-c_bar)^(alpha - 2)*(z * alpha * k_bar^(alpha-1) + 1 - delta);
J2_22 = (1 - delta)*beta + z * alpha * beta * ((z*k_bar^alpha + (1 - delta)*k_bar-c_bar)^(alpha-1)-c_bar*(alpha - 1)*(z*k_bar^alpha + (1 - delta)*k_bar - c_bar)^(alpha-2));
