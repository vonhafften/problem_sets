...
Now, let us turn to agents born in generations $t > 0$. Define $m_{t}^{t'} \in \R$ as the monetary holding by agents born in generation $t'$ that recieves an endowment while young between periods $t-1$ and $t$.\footnote{I am interpreting "there is no savings technology" to mean that agents cannot save the consumption between periods, but that they can save the fiat currency.} If $m_{t}^{t'} > 0$, then $m_{t}^{t'}$ represents the holding of fiat currency. Assuming there is a commitment technology that enforces agents paying back IOUs to other agents, then $m_{t}^{t'} < 0$ represents inside money. Thus, the problem facing agents born in generation $t > 0$ that are endowed when young is

\begin{align*}
\max_{(a_t^t, a_{t+1}^t) \in \R^2_+; m_{t+1}^t \in \R} & \ln (a_t^t) + \ln (a_{t+1}^t) \\
\text{s.t. } & p_t a_t^t \le p_t w_1- m_{t+1}^t \\ 
             & p_{t+1} a_{t+1}^t \le m_{t+1}^t \\
\end{align*}

Similarly, define $l_{t}^{t'} \in \R$ as the monetary holding by agents born in generation $t'$ that recieves an endowment while old between periods $t-1$ and $t$.  The problem facing agents born in generation $t > 0$ that are endowed when old is

\begin{align*}
\max_{(b_t^t, b_{t+1}^t) \in \R^2_+; l_{t+1}^t \in \R} & \ln (b_t^t) + \ln (b_{t+1}^t) \\
\text{s.t. } & p_t b_t^t \le -l_{t+1}^t \\ 
             & p_{t+1}b_{t+1}^t \le p_{t+1}w_2 + l_{t+1}^t \\
\end{align*}

### Competitive Equilibrium:

A competitive equilibrium is the allocation $\{(a_t^t, a_t^{t-1}, b_t^t, b_t^{t-1})\}_{\forall t}$ and prices $\{p_t\}_{\forall t}$ such that agents optimize and markets clear:

\begin{align*}
a_t^t+a_t^{t-1} +b_t^t+b_t^{t-1} &= \frac{w_1 + w_2}{2} & \text{(Goods Market)} \\
m_{t+1}^t + l_{t+1}^t &= M & \text{(Money Market)}
\end{align*}
