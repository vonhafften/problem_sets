...
For the consumer problem, I break it up into the problem of four different types of agents laid out in the model setup: an initial old person that does not recieve an endowment, an initial old person that does receive an endowment, an agent in any generation $t > 0$ that recieves an endowment when young, and an agent in any generation $t > 0$.

First, let us consider the problem of an initial old person without an endowment.  This agent's problem is
...
Second, let us consider the problem of an initial old person that recieves an endowment.  This agent's problem is
...
Now, let us turn to agents in generations $t > 0$.  Notice that it is possible for an agent in the same generation to agree to an IOU between. Define $m_{t+1}^t$ as the holding of fiat currency between periods $t$ and $t+1$.

\begin{align*}
\max_{asdfas} \ln (c_t^t) + \ln (c_{t+1}^t) \\
\text{s.t. } 
\end{align*}
