...
\begin{pmatrix} e_{11} & e_{12} \\ e_{21} & e_{22} \end{pmatrix}  \begin{pmatrix} a_1 \lambda_1^t \\ a_2 \lambda_2^t \end{pmatrix}
\end{align*}

The system has a saddle path because the absolute value of one eigenvalue is greater than one and the absolute value of the other eigenvalue is less than one. The saddle path is $(k_t, c_t)$ where $a_1 = 0$.

\begin{align*}
\begin{pmatrix} k_t \\ c_t \end{pmatrix} = 
\begin{pmatrix} e_{11} & e_{12} \\ e_{21} & e_{22} \end{pmatrix}  \begin{pmatrix} 0 \\ a_2 \lambda_2^t \end{pmatrix}
= \begin{pmatrix} e_{12}a_2 \lambda_2^t \\ e_{22}a_2 \lambda_2^t \end{pmatrix}
\end{align*}

So, the slope of the saddle path at the steady state is $\frac{e_{22}a_2 \lambda_2^t}{e_{12}a_2 \lambda_2^t} = \frac{e_{22}}{e_{12}}=\frac{0.1734}{0.9848}=0.1761$.
...
plot(1, type ="n", xlim=c(3, 4), ylim=c(1, 1.3), xaxs="i", yaxs="i", xlab="k_t", ylab="c_t",  main = "Phase Diagram with Saddle Paths")
abline(h=0)
...
points(x=k_bar, y=c_bar, pch = 4, col = "red", cex=2)
points(x=k_bar2, y=c_bar2, pch = 8, col = "red", cex=2)
...
$$(\bar{k}', \bar{c}') \approx (3.7458, 1.2424)$$

The new Jacobian is

$$J = \begin{pmatrix} 1.0440 & -1 \\ -0.0319 & 1.0392 \end{pmatrix}$$

(b) Diagonalize the system using eigenvectors and rewrite it in terms of $\hat{k}_t$ and $\hat{c}_t$.

From Matlab:

The new matrix of eigenvalues is:

$$ \Lambda \approx \begin{pmatrix} 1.2203 & 0 \\ 0 & 0.8629 \end{pmatrix}$$

The new matrix of eignvectors is:

$$ E \approx \begin{pmatrix} 0.9848 & 0.9840 \\ -0.1736 & 0.1783 \end{pmatrix}$$
