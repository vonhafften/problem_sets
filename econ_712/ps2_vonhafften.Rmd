...
For the consumer problem, I break it into the three problems: the problem facing an initial old agent, the problem facing an initial mid agent, and the problem facing all agents born in generation $t > 0$.  Let $p_t$ be the number of dollar per unit of consumption good in period $t$.  Define $m_{t+1}^t \in \R$ as the monetary holdings of the generation born in $t$ between periods $t$ and $t+1$ and define $m_{t+1}^t \in \R$ as the monetary holdings of the generation born in $t$ between periods $t+1$ and $t+2$.

The problem of the initial old agent is

\begin{align*}
\max_{c_1^{-1} \in \R_+} & \ln (c_1^{-1}) \\
\text{s.t. } & p_1c_1^{-1} \le p_1w_3 + M
\end{align*}
...
A competitive equilibrium is the allocation $\{(c_t^t, c_t^{t-1}, c_t^{t-2}\}_{\forall t}$ and prices $\{p_t\}_{\forall t}$ such that agents optimize:

\begin{align*}
\max_{(m_{t+1}^t, m_{t+2}^t) \in \R^2} & \ln \Bigg( w_1- \frac{m_{t+1}^t}{p_t} \Bigg) + \ln \Bigg(w_2 + \frac{m_{t+1}^t-m_{t+2}^t}{p_{t+1}}  \Bigg) + \ln \Bigg( w_3 + \frac{m_{t+2}^t}{p_{t+2}} \Bigg) 
\end{align*}

and markets clear:

\begin{align*}
n^2c_t^t + nc_t^{t-1} + c_t^{t-2} &= n^2 w_1 + nw_2 + w_3 \\
n m_{t}^{t-1} + m_{t}^{t-2} &= \frac{N_{-1}}{N_{t-2}}.
\end{align*}
