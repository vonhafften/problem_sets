...
We use backward induction.  Given $x_i$, $x_j$, $p_i$, and $p_j$, consumers buy their coffee from coffee shop $i$ if

\begin{align*}
c(w - x_i)^2 + p_i &< c(w - x_j)^2 + p_j \\
\implies c(w^2 - 2x_iw +x_i^2) + p_i &< c(w^2 - 2x_jw +x_j^2) + p_j \\
\implies cw^2 - 2cx_iw + cx_i^2 + p_i &< cw^2 - 2cx_jw + cx_j^2 + p_j \\
\implies w &<  \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}
\end{align*}

Thus, coffee shop $i$ sells to the following quantity of consumers:

$$
\int_0^1 1\{ w  <  \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}\} dw = \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)} 
$$

Because $w$ is uniformly distributed on $[0, 1]$. Thus, the total revenue for coffee shop $i$ is
...
Guess that pricing is symmetric in the subgame perfect nash equilibrium is symmetric:
