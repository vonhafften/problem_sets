...
We use backward induction by first considering the coffee lovers choice of coffee shop given $x_1$, $x_2$, $p_1$, and $p_2$, then the pricing decision by the coffee shops given $x_1$ and $x_2$, and finally the location decision by the coffee shops.

First, consider the coffee lovers choice of coffee shop given $x_1$, $x_2$, $p_1$, and $p_2$.  Consumers are indifferent between coffee shop $i$ and $j$ if
...
\implies cw^2 - 2cx_iw + cx_i^2 + p_i &= cw^2 - 2cx_jw + cx_j^2 + p_j \\
\implies 2cw(x_j-  2cx_i)  &= c(x_j^2 - x_i^2) + p_j - p_i \\
\implies w &=  \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}
\end{align*}

Thus, if $x_i < x_j$, then coffee shop $i$ serves the consumers located below the cutoff:

$$
w \in \Bigg[0, \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}\Bigg)
$$

So coffee shop $i$ serves this quantity of consumers:

$$
\int_0^1 1\{ w  <  \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}\} dw = \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)} 
$$

Because $w$ is uniformly distributed on $[0, 1]$. If $x_i > x_j$, then coffee shop $i$ serves the consumer located above the 

$$
w \in \Bigg[\frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}, 1\Bigg]
$$

So coffee shop $i$ serves this quantity of consumers:

$$
\int_0^1 1\{ w  >  \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}\} dw = 1 - \frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)} 
$$

If $x_i = x_j$ and $p_i < p_j$, then coffee shop $i$ serves the entire mass of costumers.  If $x_i = x_j$ and $p_i > p_j$, then coffee shop $j$ serves the entire mass of costumers and coffee shop $i$ does not serve anyone.  If $x_i = x_j$ and $p_i = p_j$, then each coffee shop serves coffee to 1/2 because everyone along the $[0, 1]$ interval is indifferent between going to either shop and flips a coin.  

\pagebreak

The table below summarizes the quantity of consumers served by coffee shop $i$ given $x_i$, $x_j$, $p_i$, and $p_j$:

+--------------+---------------------------------------------------+----------------------------+-----------------------------------------------------+
|              | $x_i < x_j$                                       | $x_i = x_j$                | $x_i > x_j$                                         |
+--------------+---------------------------------------------------+----------------------------+-----------------------------------------------------+
| $p_i > p_j$  | $\frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}$ |  0                         | $1-\frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}$ |
+--------------+---------------------------------------------------+----------------------------+-----------------------------------------------------+
| $p_i = p_j$  | $\frac{c(x_j^2-x_i^2)}{2c(x_j- x_i)}$             |  1/2                       | $1-\frac{c(x_j^2-x_i^2)}{2c(x_j- x_i)}$             |
+--------------+---------------------------------------------------+----------------------------+-----------------------------------------------------+
| $p_i < p_j$  | $\frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}$ |  1                         | $1-\frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)}$ |
+--------------+---------------------------------------------------+----------------------------+-----------------------------------------------------+

Second, let us consider the pricing decision by the coffee shops given $x_i$ and $x_j$.  First, consider the case in which $x_i = x_j$.  Clearly, each coffee shop has the incentive to undercut the other to serve the entire unit mass so $p_i = p_j = 0$.

Now let us evaluate the best response function for coffee shop $i$ based on $x_i$, $x_j$, and $p_j$ when $x_i \neq x_j$. If $x_i < x_j$, the total revenue for coffee shop $i$ is:
...
p_i^*(x_i, x_j, p_j) &= \frac{c(x_j^2-x_i^2) + p_j}{2}
\end{align*}

If $x_i > x_j$, the total revenue for coffee shop $i$ is:

$$
p_i(1-\frac{c(x_j^2-x_i^2) + p_j - p_i}{2c(x_j- x_i)})
$$

FOC [$p_i$]:

\begin{align*}
0 &= 1-\frac{c(x_j^2-x_i^2) + p_j - 2p_i^*}{2c(x_j- x_i)} \\
2c(x_j- x_i) &= c(x_j^2-x_i^2) + p_j - 2p_i^* \\
p_i^*(x_i, x_j, p_j) &= \frac{c(x_j^2-x_i^2) - 2c(x_j- x_i) + p_j}{2}
\end{align*}

\pagebreak

Now let us combine these best response functions so that both coffee shops are best responding to each other. If $x_i < x_j$, we can use the first best response function for $i$ and the second best response function for $j$. If both coffee shops are best responding to each other we can substitute in the best response function:

\begin{align*}
p_i^* &= \frac{c(x_j^2-x_i^2) + (\frac{c(x_i^2-x_j^2) - 2c(x_i- x_j) + p_i^*}{2})}{2} \\
\implies p_i^* &= \frac{c(x_j^2-x_i^2)}{2} + \frac{c(x_i^2-x_j^2)}{4} - \frac{c(x_i- x_j)}{2} + \frac{p_i^*}{4} \\
\implies \frac{3p_i^*}{4} &= \frac{c(x_j^2-x_i^2)}{4}  - \frac{c(x_i- x_j)}{2} \\
\implies p_i^*(x_i, x_j) &= \frac{c(x_j^2-x_i^2)}{3}  - \frac{2c(x_i- x_j)}{3}
\end{align*}

If $x_i > x_j$, coffee shop $j$ will price as above, so the best response function for $i$ is:

$$
p_i^*(x_i, x_j) = \frac{c(x_j^2-x_i^2) - 2c(x_j- x_i) + \frac{c(x_i^2-x_j^2)}{3}  - \frac{2c(x_j- x_i)}{3}}{2}\\
= \frac{c(x_j^2-x_i^2)}{3} - \frac{4c(x_j- x_i)}{3}
$$
