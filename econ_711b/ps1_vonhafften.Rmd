...
Let $t_i \in S_i$ be strictly dominated by $x_i \in \Delta (S_i)$, $u(x_i) > u(t_i)$.  For any $y_i \in \Delta (S_i)$ and $p \in (0,1]$, construct $\sigma_i := p t_i \oplus (1-p)y_i$.  That is, $\sigma_i$ is the mixed strategy where $t_i$ is played with probability $p$ and $y_i$ is played with probability $1-p$. Similarly, construct $z_i := p x_i \oplus (1-p)\sigma_i$. By the independence of preferences underlying the Bernoulli utility function $u$, $u(x_i) > u(t_i) \implies u(z_i) > u(\sigma_i)$.  Thus, $\sigma_i$ has $t_i$ in its support and it is strictly dominated.
...
text("Player 1's Best Response", col = "forestgreen",x = .5, y=.9)
text("Player 2's Best Response", col = "blue",x = .25, y=.5)
