...
Since there's a continuum of agents, no single agent's guess alters the average.  Let us first find an interior solution.  The FOC is

$$
0 = 2(x_i - \alpha) - 2(x_i - \bar{x})
$$

In a symmetric equilibrium, all agents guess the same number so the agent's $i$ guess is the average guess:

$$
0 = 2(x_i - \alpha) - 2(x_i - x_i)  \\
\implies x_i = \alpha
$$
...
This equation is true for $q_i^* = 0$, $q_i^* = 1$, and $q_i^* = 2$. Thus, there are pure-strategy Nash equilibriums at $(0, 0)$, $(1, 1)$, and $(2, 2)$.

In addition to pure-strategy Nash equilibriums, there's four mixed-strategy Nash equilibriums: 

a. Players mix between $(0, 0)$ and $(1, 1)$.
b. Players mix between $(1, 1)$ and $(2, 2)$.
c. Players mix between $(0, 0)$ and $(2, 2)$.
d. Players mix between $(0, 0)$, $(1, 1)$, and $(2, 2)$.

The payoffs at these levels are summarized in the table below.
...
|$q_i=0$   | $(0,0)$     | $(0, -1/2)$|$(0, -2)$ |
+----------+-------------+------------+----------+
|$q_i=1$   | $(-1/2, 0)$ |$(1/2, 1/2)$|$(3/2, 0)$|
+----------+-------------+------------+----------+
|$q_i=2$   | $(-2, 0)$   | $(0,3/2)$  |$(2, 2)$  |
+----------+-------------+------------+----------+

For a., let $\sigma_a$ be the probability that player $j$ plays 0 and $1-\sigma_a$ the probability that player $j$ plays 1. Player $i$ is indifferent between playing 0 and 1 if 

\begin{align*}
\sigma_a u_i(0, 0) + (1-\sigma_a) u_i(0, 1) &= \sigma_a u_i(1, 0) + (1-\sigma_a) u_i(1, 1) \\
\sigma_a (0) + (1-\sigma_a) (0) &= \sigma_a (-1/2) + (1-\sigma_a) (1/2) \\
\sigma_a &= 1/2  \\
\end{align*}

By symmetry, the same $\sigma_a$ holds for $j$ to be indifferent, so there's a mixed strategy Nash equilibrium of $(0, 0)$ with 1/2 probability and $(1, 1)$ with 1/2 probability.

For b., let $\sigma_b$ be the probability that player $j$ plays 1 and $1-\sigma_b$ the probability that player $j$ plays 2. Player $i$ is indifferent between playing 1 and 2 if 

\begin{align*}
\sigma_b u_i(1, 1) + (1-\sigma_b) u_i(1, 2) &= \sigma_b u_i(2, 1) + (1-\sigma_b) u_i(2, 2) \\
\implies \sigma_b (1/2) + (1-\sigma_b) (3/2) &= \sigma_b (0) + (1-\sigma_b) (2) \\
\implies \sigma_b &= 1/2 \\
\end{align*}

By symmetry, the same $\sigma_b$ holds for $j$ to be indifferent, so there's a mixed strategy Nash equilibrium of $(1, 1)$ with 1/2 probability and $(2, 2)$ with 1/2 probability.

For c., let $\sigma_c$ be the probability that player $j$ plays 0 and $1-\sigma_c$ the probability that player $j$ plays 2. Player $i$ is indifferent between playing 0 and 2 if 

\begin{align*}
\sigma_c u_i(0, 0) + (1-\sigma_c) u_i(0, 2) &= \sigma_c u_i(2, 0) + (1-\sigma_c) u_i(2, 2) \\
\sigma_c (0) + (1-\sigma_c) (0) &= \sigma_c (-2) + (1-\sigma_c) (2) \\
\sigma_c &= 1/2 \\
\end{align*}

By symmetry, the same $\sigma_c$ holds for $j$ to be indifferent, so there's a mixed strategy Nash equilibrium of $(0, 0)$ with 1/2 probability and $(2, 2)$ with 1/2 probability.

For d., notice that no matter how player $j$ mixes playing $q_j=0$, $q_j=1$, and $q_j=2$, the payoff for $i$ is zero.  Thus, the expected payoff for $i$ to play $q_i=0$ is zero.  That means that the expected payoff for $i$ to play $q_i=1$ and $q_i=2$ needs to both be zero.  For the expected payoff from $q_i=2$ to equal zero, it requires that player $j$ plays $q_j=0$ and $q_j = 2$ with equal probability.  However, if player $j$ plays $q_j=0$ and $q_j = 2$ with equal probability, that means the expected payoff for player $i$ playing $q_j = 1$ is strictly positive.  Thus, there is no mixed-strategy Nash equilibrium that combines $(0, 0)$, $(1, 1)$, and $(2, 2)$.
