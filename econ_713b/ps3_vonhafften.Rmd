...
Notice that the expected level of output with low effort is insufficient to cover the bank's cost of funds even if the entrepreneur gets nothing, so the bank will never design a contract that incentives low effort.  The bank seeks to implement a high level of effort if its expected profit is positive; I provide such a condition at the end. If the entrepreneur exerts effort level $e_1$, her expected payoff is:

$$
xP_1 - c_1
$$

If the entrepreneur exerts effort level $e_2$, her expected payoff is:
...
Notice that the incentive compatibility constraint is tighter than the participation constraint.  In addition, the bank wants to maximize its expected profits, so it chooses the smallest $x$ to incentive high effort, so:
