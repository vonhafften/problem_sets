...
Assuming that $\alpha$ and $\alpha + \gamma$ are positive, an increase in $R$ leads to be increase in $n_t$.

Let's solve for a competitive equilibrium.  Let $\lambda_t$ be the multiplier on the HH budget constraint and $\xi_t$ be the multiplier on the cash-in-advance constraint:

\begin{align*}
\sum_{t=0}^\infty & \beta^t [\log c_{1,t}+\alpha \log c_{2,t} + \gamma \log(1 - n_t)] \\
...
Assuming $\alpha$ is positive, an increase in $R$ increases the numerator.  Assuming $\alpha + \gamma$ is positive, an increase in $R$ decreases the denominator. Thus, an increase in $R$ increases $n_t$.
